...
//
// Also includes a SKU-level detail report (below the per-store table)
// pulled from ScrapeRunSkuMatches — shows PP/isActive/isInStock from
// InternalProducts and CompetitorPrice/StockStatus/ScrapedAt from the
// competitor scrape, per SKU. Defaults to Recommendation Match + Basic
// Match rows only (the "No SKU" / "No SKU Match" buckets are usually
// huge and less interesting for this view), with a toggle to see all.
// ─────────────────────────────────────────────────────────────
...
import {
  ...
  fetchScrapeRunSkuMatches,
  downloadScrapeRunSkuMatchesCsv,
} from '../services/api';
...
// ── Small Yes/No badge for BIT columns (IsActive / IsInStock). Handles
// null (no internal match at all, so the field genuinely doesn't apply). ──
function boolBadge(v) {
  if (v === null || v === undefined) return <span className="text-slate-600">—</span>;
  return v ? <span className="text-emerald-300">Yes</span> : <span className="text-red-300">No</span>;
}
...
export default function ScrapeStatsView({ onClose }) {
  ...
  // ── SKU-level detail report state ──────────────────────────────
  const [skuRows, setSkuRows]         = useState([]);
  const [skuLoading, setSkuLoading]   = useState(false);
  const [skuError, setSkuError]       = useState(null);
  const [skuFilter, setSkuFilter]     = useState('matched'); // 'matched' | 'all'
  ...
  const loadSkuMatches = useCallback(async (which) => {
    setSkuLoading(true);
    setSkuError(null);
    try {
      const data = await fetchScrapeRunSkuMatches(which);
      setSkuRows(data.rows || []);
    } catch (err) {
      setSkuError(err?.response?.data?.error || err.message || 'Failed to load SKU detail report');
    } finally {
      setSkuLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    loadRuns();
    loadDetail('latest');
    loadSkuMatches('latest');
  }, [loadRuns, loadDetail, loadSkuMatches]);

  function handleRunChange(e) {
    const value = e.target.value;
    setSelectedRunId(value);
    loadDetail(value);
    loadSkuMatches(value);
  }
...
//
  ...
  // ── Filtered SKU-level rows for the detail report ───────────────
  const filteredSkuRows = skuRows.filter(r => {
    if (skuFilter === 'matched' && !['Recommendation Match', 'Basic Match'].includes(r.MatchStatus)) return false;
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-[1400px] mx-auto px-6 pr-16 sm:pr-20 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { loadRuns(); loadDetail(selectedRunId); loadSkuMatches(selectedRunId); }} disabled={loading}
  ...
  {downloading ? 'Preparing...' : 'Download CSV'}
            ...
            {/* ── SKU-level detail report — per-SKU PP/isActive/isInStock
                from InternalProducts and CompetitorPrice/StockStatus/ScrapedAt
                from the competitor scrape. Defaults to Recommendation Match +
                Basic Match rows only; toggle to see No SKU / No SKU Match too. ── */}
            <div className="mt-8">
              <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                <h2 className="text-sm font-semibold text-slate-200">SKU-Level Detail Report</h2>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                  ...
                  <select
                    ...
                    className="bg-slate-800 border border-slate-700 text-xs text-slate-200 rounded-lg
                      px-3 py-1.5 focus:outline-none focus:border-violet-500"
                  >
                    <option value="matched">Recommendation + Basic Match only</option>
                    <option value="all">All rows (incl. No SKU / No SKU Match)</option>
                  </select>
                </div>
              </div>

              {skuLoading && (
                <div className="text-xs text-slate-500 py-6 text-center">Loading SKU detail report...</div>
              )}
              {skuError && (
                <div className="p-3 rounded-xl bg-red-900/30 border border-red-700/50 text-red-400 text-xs mb-3">{skuError}</div>
              )}
                    ...
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-slate-800/95 backdrop-blur">
                        <tr className="border-b border-slate-700">
                          {['SKU', 'Store', 'TPS Category', 'Match Status', 'PP', 'Active', 'In Stock',
                            'Competitor Price', 'Competitor Stock', 'Scraped At', 'Reason'].map((h, idx) => (
                            <th key={h}
                              className={`px-2 py-2 font-semibold text-slate-400 uppercase tracking-wider
                                ${idx === 0 ? 'text-left' : 'text-center'}`}
                            >
                              {h}
                            </th>
                      ...
                      <tbody>
                        {filteredSkuRows.map((row, i) => (
                          <tr key={i} className="border-b border-slate-800/60">
                            <td className="px-2 py-2 text-left text-slate-200 font-medium">{row.SKU}</td>
                            <td className="px-2 py-2 text-center text-slate-400">{row.StoreName}</td>
                            <td className="px-2 py-2 text-center text-slate-400">{row.CategoryNames || '—'}</td>
                            <td className={`px-2 py-2 text-center font-medium ${
                              row.MatchStatus === 'Recommendation Match' ? 'text-emerald-300' :
                              row.MatchStatus === 'Basic Match' ? 'text-sky-300' : 'text-amber-300'
                            }`}>{row.MatchStatus}</td>
                            <td className="px-2 py-2 text-center text-slate-300">
                              {row.PP != null ? `₹${row.PP}` : '—'}
                            </td>
                            <td className="px-2 py-2 text-center">{boolBadge(row.IsActive)}</td>
                            <td className="px-2 py-2 text-center">{boolBadge(row.IsInStock)}</td>
                            <td className="px-2 py-2 text-center text-slate-300">
                              {row.CompetitorPrice != null ? `₹${row.CompetitorPrice}` : '—'}
                            </td>
                            <td className="px-2 py-2 text-center text-slate-400">{row.CompetitorStockStatus || '—'}</td>
                            <td className="px-2 py-2 text-center text-slate-500">{fmtDate(row.ScrapedAt)}</td>
                            <td className="px-2 py-2 text-left text-slate-500">{row.Reason || '—'}</td>
                          </tr>
                        ))}
